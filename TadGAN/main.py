...
if __name__ == "__main__":
    dataset = pd.read_csv('./UCR/028_UCR_Anomaly_DISTORTEDInternalBleeding17_1600_3198_3309.txt')
    #Splitting intro train and test
    #TODO could be done in a more pythonic way
    train_len = 1600  # Mark: match the first number in file description
    dataset[0:train_len].to_csv('train_dataset.csv', index=False)
    dataset[2000:4000].to_csv('test_dataset.csv', index=False)

    trainset = pd.read_csv('train_dataset.csv')
    trainset['anomaly'] = 0  # Mark: training does not have anomalies
    trainset.to_csv('train_dataset.csv', index=False)

    testset = pd.read_csv('test_dataset.csv')
    # Mark: match the file description, anomaly is between second and third number from file name.
    testset['anomaly'] = [0] * 1198 + [1] * 111 + [0] * 691
    ...
    train(n_epochs=10)  # Mark: adjust to increase/decrease training epochs (takes a long time)
