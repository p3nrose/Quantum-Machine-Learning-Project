...
def detect_anomaly(anomaly_score):
    window_size = len(anomaly_score) // 15  # Mark: adjust to make window size close to 128
    print(window_size)
    step_size = len(anomaly_score) // (15 * 10)  # Mark: adjust to match window size
    ...
    for i in range(0, len(anomaly_score) - window_size, step_size):
        ...
        for j, elt in enumerate(window_elts):
            # Mark: can adjust number to more easily classify as anomaly
            if (window_mean - 1.5 * window_std) < elt < (window_mean + 1.5 * window_std):
                is_anomaly[i + j] = 0
            else:
                is_anomaly[i + j] = 1
...
def find_scores(y_true, y_predict):
    ...
    precision = tp / (tp + fp)  # Mark: needs to be commented out if TP is expected to be 0
    recall = tp / (tp + fn)
